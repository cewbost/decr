...
contract("Aspect", accounts => {
  ...
  describe("request", () => {
    beforeEach(async () => {
      await testAspect.addGeneration(asEthWord(1), now, now + day, [])
      await testAspect.addGeneration(asEthWord(2), now, now + day, [])
      await testAspect.addGeneration(asEthWord(3), now - 2 * day, now - day, [])
      await testAspect.addGeneration(asEthWord(4), now + day, now + 2 * day, [])
    })
    it("should store multiple distinct records", async () => {
      ...
      recs = await testAspect.getRecordsByGeneration(asEthWord(1))
      expect(recs.map(objectify)).to(consistOf([match1, match3, match5, match7]))
      recs = await testAspect.getRecordsByGeneration(asEthWord(2))
  ...
  describe("grant", () => {
    ...
    beforeEach(async () => {
      await testAspect.addGeneration(asEthWord(1), now - day, now + day, [])
      ...
      await testAspect.addPendingRecord(
        accounts[1],
        asEthWord(1),
        now - 10 * day,
        asEthBytes("details", 24),
      ...
      hash = (await testAspect.getRecordsByGeneration(asEthWord(1)))[0].hash
    })
    it("should emit grant event and remove the record from pending", async () => {
      ...
      expect(await testAspect.getRecordsByGeneration(asEthWord(1))).to(beEmpty())
    })
    ...
    it("should not allow granting after generation is expired", async () => {
      await testAspect.addGeneration(asEthWord(2), now - 2 * day, now - day, [])
      await testAspect.addPendingRecord(
        accounts[1],
        asEthWord(2),
        now - 15 * day,
      ...
      hash = (await testAspect.getRecordsByGeneration(asEthWord(2)))[0].hash
  ...
  describe("approve", () => {
    ...
    beforeEach(async () => {
      ...
      await testAspect.addGeneration(asEthWord(1), now, now + day, accounts.slice(4, 6))
      await testAspect.addPendingRecord(
        accounts[1],
        asEthWord(1),
        now - 10 * day,
    ...
    it("should add approvals to the record", async () => {
      ...
      let resp = (await testAspect.getRecordsByGeneration(asEthWord(1))).map(objectify)
    ...
    it("should not allow approving after generation is expired", async () => {
      await testAspect.addGeneration(asEthWord(2), now - 2 * day, now - day, [accounts[2]])
  ...
  describe("newGeneration", () => {
    ...
    it("should not allow creating generations with end timestamp before beginning", async () => {
      expect(await awaitException(() => {
        return testAspect.newGeneration(asEthWord(1), now + 2 * day, now + day, fromOwner)
      })).to(beVMException("ending must be before beginning"))
    })
    it("should not allow creating expired generations", async () => {
      expect(await awaitException(() => {
        return testAspect.newGeneration(asEthWord(1), now - day, now, fromOwner)
      })).to(beVMException("generation must not be expired"))
    })
  ...
  describe("clearGeneration", () => {
    beforeEach(async () => {
      await testAspect.addGeneration(asEthWord(1), now - 2 * day, now - day, [])
      await testAspect.addGeneration(asEthWord(2), now - 2 * day, now - day, [])
      await testAspect.addGeneration(asEthWord(3), now - day, now + day, [])
      await testAspect.addPendingRecord(
        accounts[1],
        asEthWord(1),
        now - 10 * day,
    ...
    it("should remove pending records from cleared generation", async () => {
      ...
      resp = (await testAspect.getRecordsByGeneration(asEthWord(2))).map(objectify)
  ...
  describe("enableApproverForGeneration", async () => {
    ...
    it("should fail if generation expired", async () => {
      await testAspect.addGeneration(asEthWord(2), now - 2 * day, now - day, [])
      expect(await awaitException(() => {
        return testAspect.enableApproverForGeneration(accounts[1], asEthWord(2))
  ...
  describe("disableApproverForGeneration", async () => {
    beforeEach(async () => {
      ...
      await testAspect.addGeneration(asEthWord(1), now, now + day, accounts.slice(1, 4))
    })
    it("should disable approvers for a generation", async () => {
